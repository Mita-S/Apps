...
decides who gets the Review page.

Setup: see SETUP.md. Until `[auth]` exists in .streamlit/secrets.toml,
`st.user.is_logged_in` is always False, so `require_login()` will show the
sign-in screen forever — that's expected before Google OAuth is wired up,
not a bug.
"""
...
# DATASET_DEV_NO_AUTH=1 substitutes a fake signed-in user so a headless
# browser can reach the Collect and Review pages.
#
...
DEV_USER_EMAIL = os.environ.get("DATASET_DEV_EMAIL", "dev@localhost")
...
def _dev_no_auth() -> bool:
    ...
    if os.environ.get("DATASET_DEV_NO_AUTH") != "1":
        return False
    if _in_secrets("DATASET_DEV_NO_AUTH"):
...
def _is_logged_in() -> bool:
    """Whether Streamlit has a signed-in user right now.

    `st.user.is_logged_in` raises AttributeError rather than returning False
    in contexts where the user object was never populated (no [auth] section
    yet, or a test harness). Reading it straight would replace the sign-in
    screen with a stack trace in exactly the situation the sign-in screen
    exists to handle.
    """
    try:
...
def require_login() -> None:
    ...
    if _dev_no_auth():
        st.warning(
            "⚠️ **DATASET_DEV_NO_AUTH=1** — sign-in is bypassed and you are "
    ...
    st.markdown("### 🔢 Build a Digit Dataset")
    st.write("Sign in with your Google account to contribute handwritten digits.")
    ...
    if auth_configured:
        ...
        st.error(
            ...
            "`.streamlit/secrets.toml` — step-by-step guide in **SETUP.md**."
        )
    st.stop()
...
# Session keys holding the current user's working state. They are dropped on
# log-out: st.session_state survives the identity change, so the canvas and
# the "you just saved a 7" banner would otherwise carry over to whoever signs
# in next.
_USER_STATE_KEYS = ("canvas_key", "digit_label", "last_saved", "saved_this_session")


def clear_user_state() -> None:
    """Forget the current user's working state."""
    for key in _USER_STATE_KEYS:
        st.session_state.pop(key, None)


def logout() -> None:
    """Log the user out, clearing their working state first."""
    clear_user_state()
    if _dev_no_auth():
...
def user_badge() -> None:
    ...
    with st.sidebar:
        ...
        n_saved = st.session_state.get("saved_this_session", 0)
        if n_saved:
            st.caption(f"✅ {n_saved} digit(s) saved this session.")
        ...
        st.button(
            ...
            help=(
                "Resets this session. Sign-in is bypassed "
                "(DATASET_DEV_NO_AUTH=1), so there is no Google session to end."
                if _dev_no_auth()
                else "Ends your Google session for this app."
            ),
        )

        if st.session_state.pop("_dev_logout_notice", False):
            st.info("Session reset. Real sign-out needs DATASET_DEV_NO_AUTH unset.")
